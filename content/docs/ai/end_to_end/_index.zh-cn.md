---
title: 2. 端到端的机器学习
weight: 112
#next: /step1
#prev: /docs
editURL: "https://example.com/edit/this/page"
toc: true
---

<br>

<div style="display: flex; justify-content: center; width: 100%;">
<div style="flex: 20%; padding: 10px; box-sizing: border-box; text-align: left;">
{{< card link="overview" title="AI 概述" subtitle="人工智能，机器学习，深度学习，统计机器学习等相关概念；" icon="package" >}}<br>
</div>
<div style="flex: 20%; padding: 10px; box-sizing: border-box; text-align: left;">
{{< card link="algorithms" title="常用算法" subtitle="了解常用算法原理。了解算法优缺点，原理，应用；" icon="package" >}}<br>
</div>
<div style="flex: 20%; padding: 10px; box-sizing: border-box; text-align: left;">
{{< card link="CRISP-DM" title="CRISP-DM" subtitle="选择并学习一个过程。学习建模分步过程；" icon="package" >}}<br>
</div>
<div style="flex: 20%; padding: 10px; box-sizing: border-box; text-align: left;"></div>
</div>
